...
export default async function SettingsPage() {
  ...
  const [subscriptions, plan] = await Promise.all([
    userId
      ? prisma.subscription.findMany({
          ...
          include: { newsletter: { select: { id: true, title: true, slug: true, categoryId: true, frequency: true, scheduleDays: true, scheduleHour: true } } },
  ...
  const subscriptionData = subscriptions.map((s) => ({
    ...
    scheduleDays: s.scheduleDays.length > 0 ? s.scheduleDays : s.newsletter.scheduleDays,
